//+------------------------------------------------------------------+
//| BTC_Bear_EMA_Ribbon.mq5                                          |
//| Strategy: EMA Ribbon BEARISH Scalper                             |
//| Asset: BTCUSD | Timeframe: M5 | Session: NY + London             |
//| Logic: 4-EMA ribbon (8/13/21/34) all aligned downward.          |
//|        Price bounces up to EMA8 zone then next candle shows      |
//|        bearish close below EMA8 = sell.                          |
//| Magic: 200002                                                    |
//+------------------------------------------------------------------+
...
input int    InpE3 = 21;
input int    InpE4 = 34;
input double InpPullbackATR = 0.4;
...
void OnTick()
{
   ...
   bool bearClose = (SC_Close(_Symbol, PERIOD_M5, 1) < SC_Open(_Symbol, PERIOD_M5, 1));

   if (ribbon && nearE1 && bearClose)
   {
      ...
      g_trade.Sell(lots, _Symbol, bid, sl, tp, "BTC_Bear_EMA_Ribbon");
   }
}
