//+------------------------------------------------------------------+
//| BTC_NY_Session_Bear.mq5                                          |
//| Strategy: New York Session BEARISH BTC Scalper                   |
//| Asset: BTCUSD | Timeframe: M5 | Session: NY 13:00-22:00 UTC      |
//| Logic: Stochastic K crosses D while above 50 and RSI < 55 =     |
//|        sell the intraday rally during NY session downtrend.      |
...
void OnTick()
{
   ...
   bool stochSell = (k1 < d1 && k2 >= d2 && k1 > InpStoch_Min);
   bool downtrend = (ema21 < ema50);
   bool rsiOK     = (rsi < 55);

   if (stochSell && downtrend && rsiOK)
   {
      ...
      g_trade.Sell(lots, _Symbol, bid, sl, tp, "BTC_NY_Session_Bear");
   }
}
