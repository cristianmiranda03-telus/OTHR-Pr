//+------------------------------------------------------------------+
//| GOLD_Bear_ATR_Expansion.mq5                                      |
//| Strategy: ATR Volatility Expansion BEARISH Breakdown Scalper     |
//| Asset: XAUUSD | Timeframe: M5 | Session: London + NY             |
//| Logic: When ATR expands above its 20-bar SMA (volatility surge), |
//|        and price closes below the 20-bar lowest low (Donchian)   |
//|        = sell the explosive move downward.                       |
...
input int    InpATR_SMA_Period  = 20;
input double InpATR_ExpMult     = 1.3;
input int    InpDonchian_Period = 20;
...
void OnTick()
{
   ...
   double donchLow = SC_GetLowestLow(_Symbol, PERIOD_M5, InpDonchian_Period, 2);
   ...
   bool donchBreak   = (close < donchLow);
   bool downSlope    = (ema50 < ema50_2);

   if (volExpansion && donchBreak && downSlope)
   {
      ...
      g_trade.Sell(lots, _Symbol, bid, sl, tp, "GOLD_Bear_ATR_Expansion");
   }
}
