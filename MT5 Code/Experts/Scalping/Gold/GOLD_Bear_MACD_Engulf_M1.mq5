//+------------------------------------------------------------------+
//| GOLD_Bear_MACD_Engulf_M1.mq5                                     |
//| Estrategia: doble confirmación MACD bajista + Power Engulf M1   |
//| Activo: XAU | TF: M1 | Sesión: London + NY                       |
//| Magic: 100015                                                    |
//+------------------------------------------------------------------+
#property copyright "Scalping Suite - GOLD M1"
#property version   "1.00"
#property description "MACD Bear + Vol Engulf Power - M1 scalping"
...
input bool   InpMACD_CrossOnly = false;

input group "=== Engulf Power M1 ==="
input double InpMinBodyATR     = 0.6;
...
{
   if (!IsXAU())
      Print("GOLD_Bear_MACD_Engulf_M1: recomendado en XAU/GOLD.");
...
void OnTick()
{
   ...
   if (rsi < InpRSI_Min) return;

   if (prevBull && currBear && engulfs && bigBody && volOK)
   {
      double bid    = SymbolInfoDouble(_Symbol, SYMBOL_BID);
      int    digits = (int)SymbolInfoInteger(_Symbol, SYMBOL_DIGITS);
      double sl     = NormalizeDouble(high1 + atr * 0.25, digits);
      double slD    = sl - bid;
      if (slD <= 0) return;
      double tp     = NormalizeDouble(bid - slD * InpTP_RR, digits);
      double slPts  = slD / SymbolInfoDouble(_Symbol, SYMBOL_POINT);
      double lots   = SC_CalcLotSize(slPts, InpRiskPct, InpMinLot, InpMaxLot);
      g_trade.Sell(lots, _Symbol, bid, sl, tp, "GOLD_MACD_Engulf_M1_S");
   }
}
