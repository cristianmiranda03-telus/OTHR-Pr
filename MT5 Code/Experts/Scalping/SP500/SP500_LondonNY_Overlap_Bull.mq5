//+------------------------------------------------------------------+
//| SP500_LondonNY_Overlap_Bull.mq5                                  |
//| Strategy: London/NY Overlap BULLISH Scalper                      |
//| Asset: US500/SP500 | Timeframe: M5 | Session: 13:00-16:00 UTC   |
//| Logic: London/NY overlap (13:00-16:00 UTC) is highest liquidity. |
//|        EMA5 > EMA13 + MACD histogram positive + RSI 50-70 =     |
//|        buy momentum during peak liquidity window.                |
//| Magic: 300006                                                    |
//+------------------------------------------------------------------+
...
void OnTick()
{
   ...
   bool bullish   = (ema5 > ema13);
   bool histPos   = ((macd1 - sig1) > 0 && (macd2 - sig2) <= 0); // fresh positive
   bool rsiOK     = (rsi >= 50 && rsi <= 70);

   if (bullish && histPos && rsiOK)
   {
      ...
      g_trade.Buy(lots, _Symbol, ask, sl, tp, "SP500_LonNY_Overlap_Bull");
   }
}
